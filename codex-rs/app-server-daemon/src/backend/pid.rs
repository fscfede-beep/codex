...
impl PidBackend {
    ...
    async fn pid_matches_expected_command(&self, pid: u32) -> Result<bool> {
        ...
        Ok(argv.len() == args.len() + 1
            ...
            && argv[1..].iter().zip(args).all(|(actual, expected)| actual == expected))
    }
    ...
    async fn pid_matches_expected_command(&self, pid: u32) -> Result<bool> {
        ...
        let expected = fs::canonicalize(&self.codex_bin).await.unwrap_or_else(|_| self.codex_bin.clone());
        let actual = fs::canonicalize(process.executable_path()?).await.unwrap_or_else(|_| self.codex_bin.clone());
        Ok(actual == expected)
    }
