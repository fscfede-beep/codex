...
impl Process {
    ...
    pub(super) fn executable_path(&self) -> Result<std::path::PathBuf> {
        ...
        loop {
            ...
            let mut len = size;
            if unsafe { QueryFullProcessImageNameW(self.0.as_raw_handle() as _, 0, buffer.as_mut_ptr(), &mut len) } != 0 {
                buffer.truncate(len as usize);
                return Ok(std::path::PathBuf::from(String::from_utf16(&buffer).context("managed process executable path was not utf-16")?));
            }
            let err = io::Error::last_os_error();
            if err.raw_os_error() == Some(122) && size < 262_144 { size *= 2; continue; }
