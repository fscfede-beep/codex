...
impl ApplyPatchRuntime {
    ...
    fn apply_options(&self, action: &ApplyPatchAction) -> ApplyPatchOptions {
        ApplyPatchOptions {
            update_file_mode: action.update_file_mode(),
            // Destructive verification uses no-follow semantics. Mutation must use the
            // exact same policy so a symlink/reparse substitution cannot reopen the path.
