    ...
    pub(crate) fn cache_keys(&self) -> Vec<ApprovalCacheKey> {
        match self {
            ...
            Self::ApplyPatch { changes, .. }
                if Self::apply_patch_requires_fresh_human_approval(changes) =>
            {
                Vec::new()
            }
            Self::ApplyPatch {
...
impl Session {
    pub(crate) async fn request_approval(
        ...
        if requires_fresh_human_approval
                ...
                ))
        {
            return Err(ToolError::Rejected(
                "destructive apply_patch is forbidden by current approval policy".to_string(),
            ));
        }
