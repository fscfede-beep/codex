...
impl ProcessExecRequestProcessor {
    ...
    pub(crate) async fn process_spawn(
        ...
        // process/spawn has no governed approval, permission profile, or sandbox
        // capability in its protocol request, so creating a host process here
        // would bypass the central execution boundary used by Codex tools.
        Err(invalid_request(
