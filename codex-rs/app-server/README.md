...
- `thread/context/attest` — experimental read-only continuity attestation. It fingerprints only visible text messages (`user` / `assistant`) from persisted thread history and from the items used by `load_latest_model_context`; it never returns message text, reasoning, tool payloads, or raw model context. An optional `anchorSha256` lets a client prove that one UI-visible message exists in persisted history and in latest model context. The message fingerprint is `SHA256(role + NUL + text)`, with visible text parts joined by newlines. `exactVisibleTextMatch` is diagnostic only because healthy compaction may intentionally replace older history; for a continuity gate, use the specific anchor or `latestVisibleTextMessageMatches` for the material the next turn depends on.
